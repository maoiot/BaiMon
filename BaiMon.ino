...
void ICACHE_RAM_ATTR uart_int_handler(void *va) {
    ...
    while (READ_PERI_REG(UART_STATUS(UART_EBUS)) & (UART_RXFIFO_CNT << UART_RXFIFO_CNT_S)) {
        RcvChar = READ_PERI_REG(UART_FIFO(UART_EBUS)) & 0xFF;
		ProcessReceive(RcvChar);
	}
}
...
void SetupEBus() {
    ...
    uart_div_modify(UART_EBUS, UART_CLK_FREQ / 2400);
    WRITE_PERI_REG(UART_CONF0(UART_EBUS), UART_8N1);

    //clear rx and tx fifo,not ready
    SET_PERI_REG_MASK(UART_CONF0(UART_EBUS), UART_RXFIFO_RST | UART_TXFIFO_RST);
    CLEAR_PERI_REG_MASK(UART_CONF0(UART_EBUS), UART_RXFIFO_RST | UART_TXFIFO_RST);

    //set rx fifo trigger = 1 byte
    WRITE_PERI_REG(UART_CONF1(UART_EBUS), (1 & UART_RXFIFO_FULL_THRHD) << UART_RXFIFO_FULL_THRHD_S);
